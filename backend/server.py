...
EMERGENT_LLM_KEY = os.environ.get('EMERGENT_LLM_KEY', 'sk-emergent-f35Fc760b5dAf1bBdC')
...
async def extract_text_from_image(image_base64: str) -> dict:
    ...
    try:
        chat = LlmChat(
            ...
            system_message="You are an expert medical prescription reader. Extract all text from prescription images accurately."
        ).with_model("openai", "gpt-4o")
        
        image_content = ImageContent(image_base64=image_base64)
        user_message = UserMessage(
            text="""Extract all text from this prescription image. Format your response as JSON:
            {
                "extracted_text": "complete text from image",
                "medicines": [
                    {"name": "medicine name", "dosage": "dosage info", "frequency": "frequency"}
                ],
...
async def upload_prescription_image(
    ...
    medicine_names = [med['name'] for med in ocr_result.get('medicines', [])]
...
async def submit_prescription_text(
    ...
    # Use LLM to parse text and extract medicines
    try:
        chat = LlmChat(
            api_key=EMERGENT_LLM_KEY,
            session_id=f"parse_{uuid.uuid4()}",
            system_message="You are a medical prescription parser."
        ).with_model("openai", "gpt-4o-mini")
        
        user_message = UserMessage(
            text=f"""Parse this prescription text and extract medicines. Format as JSON:
            {{
                "medicines": [
            ...
            Prescription: {prescription_data.text}"""
        )
...
async def chat_with_ai(chat_request: ChatRequest, user_id: str = Depends(get_current_user)):
    ...
    try:
        ...
        chat = LlmChat(
            api_key=EMERGENT_LLM_KEY,
            session_id=session_id,
            system_message=f"""You are MediAssist, a helpful AI health assistant. 
            You help patients with symptom analysis, medication information, and general health guidance.
